    ...
    >
        ...
        <div class="card-body w-full flex flex-col gap-y-4">
            ...
            >
                ...
                <small>-</small>
                <small class="text-slate-500 text-xs"
                    >{{ request.Service.Provider.User.FirstName }}
                    {{ request.Service.Provider.User.SurName }}</small
                >
                <small>-</small>
                <small class="text-slate-500 text-xs">{{
                    request.Service.Location.DisplayName
                }}</small>
                <small class="text-xs text-slate-400 italic"
                    >{{ estimateDuration(request.TimeStamp) }} ago</small
                >
            </div>
