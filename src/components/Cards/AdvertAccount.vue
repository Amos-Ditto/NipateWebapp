    ...
    <div class="advert bg-gray-50 rounded-md border border-gray-300 px-3 lg:px-8 py-3 flex flex-col gap-y-6">
        <div class="advert-top flex flex-row justify-between">
            ...
            <div class="right-top flex flex-row items-center gap-x-4 relative justify-end sm:justify-center">
                <button
                    class="border border-gray-200 rounded px-2 sm:px-4 py-0.5 sm:py-1 text-xs xs:text-sm hover:text-dodgerblue hover:border-dodgerblue transition-colors duration-300">Edit</button>
                <button
                    class="border border-gray-200 rounded px-2 sm:px-4 py-0.5 sm:py-1 text-xs xs:text-sm hover:text-tomato hover:border-tomato transition-colors duration-300"
                    @click="executeDelete">delete</button>
                <Transition name="popup">
                    <div class="modal-delete absolute  top-[120%] w-[150%] sm:w-[110%] min-h-[4rem] rounded bg-gray-50 shadow-md flex flex-col px-1.5 py-2 gap-y-1"
                        v-if="requestdelete">
                        <div class="flex flex-col gap-y-0.5">
                            <small class="text-xs text-slate-500">This action will delete "Rafiki Cafe" from all of your
                                Services.</small>
                        ...
                        <button
                            ...
                            @click="requestdelete = !requestdelete">Decline</button>
                        <button
                            ...
                            @click="requestdelete = !requestdelete">Accept</button>
                    </div>
                </Transition>
            </div>
        </div>
        <div class="advert-body w-full flex flex-col gap-y-5">
            <div class="ad-details flex flex-col gap-y-2">
                <div class="ad-location flex flex-row flex-wrap items-center w-full gap-x-2 gap-y-2">
                    <!-- <small class="text-slate-500 text-xs">Catering</small>
                            <small>-</small> -->
                    <div class="i-mdi-map-marker-outline text-slate-500"></div>
                    <small>-</small>
                    <small class="text-slate-500 text-xs">Kiamunyi, Nakuru</small>
                </div>
                <div class="ad-time flex flex-row flex-wrap items-center w-full gap-x-2 gap-y-2">
                    <div class="i-mdi-clock-outline text-sm text-slate-500"></div>
                    <small>-</small>
                    <small class="text-slate-500 text-xs">From: Today</small>
                    <small>-</small>
                    <small class="text-slate-500 text-xs">To: 07/10/2022</small>
                </div>
            </div>
