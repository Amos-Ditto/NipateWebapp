...
<template>
	<div class="register-provider flex flex-col gap-y-8 px-6 border border-gray-300 rounded-md py-6">
		...
		<div class="input-field flex flex-col gap-y-2 pt-5">
			...
			<div class="input flex items-start flex-col relative">
				<transition name="popup">
					...
					>
						<div
							class="close absolute z-40 top-2 right-2 cursor-pointer hover:bg-gray-300 p-2 rounded-full bg-gray-200"
							@click="toggleSelect"
						>
						...
						<div class="retry w-full min-h-[7rem] flex items-center justify-center" v-if="!retryfetch" @click="fetchCounties">
							<button
								class="border border-steelblue hover:bg-dodgerblue px-3 py-1 capitalize font-light text-sm text-steelblue hover:text-slate-100 transition-colors rounded tracking-wide"
								@click="fetchCounties"
							>
								retry
							</button>
				...
				<button
					...
					id="select-loc"
					@click="toggleSelect"
					class="flex flex-row items-center justify-between gap-x-8 min-w-[60%] md:min-w-[30%] px-4 py-2.5 rounded border"
					:class="validformdata ? 'border-gray-300' : 'border-tomato'"
				>
		...
		<div class="accept-terms flex flex-row gap-x-2">
			...
			<label class="text-xs" for="terms">By registering, you agree to the Terms of Service and Privacy Policy.</label>
		</div>
