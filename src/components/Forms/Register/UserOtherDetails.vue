...
const closeSelectCounty = (): void => {
	opencounty.value = false;
};
const openSelectCounty = (): void => {
	...
	>
		...
		<div class="input-fields w-full sm:w-[80%] flex flex-col gap-y-11 sm:gap-y-8 pt-14 sm:pt-12 transition-all duration-300">
			<div class="input-field w-full flex flex-col gap-y-2">
				...
				<div class="input w-full flex flex-col relative items-center justify-center">
					<button
						type="button"
						id="county"
						class="w-full rounded justify-start flex"
						:class="validlocation ? 'border-gray-300' : 'border-tomato'"
						@click="onCountyChange"
					>
						{{ county.Name }}
					</button>
					<div
						class="i-mdi-chevron-down absolute right-2 cursor-pointer hover:rotate-180 transition-transform duration-300 text-xl"
						@click="openSelectCounty"
						:class="opencounty && 'rotate-180'"
					></div>
					<Transition name="drop-down">
						<div
							class="drop-down-loc absolute px-0 py-1 -top-1/4 left-1/4 right-1/4 min-h-4 max-h-[15rem] bg-gray-50 rounded flex flex-col z-30 overflow-y-auto"
							v-if="opencounty"
						>
							<div
								class="close absolute z-40 top-1 right-3 p-2 bg-gray-200 hover:bg-gray-300 cursor-pointer rounded-full text-slate-600"
								@click="closeSelectCounty"
							>
								<div class="i-mdi-close"></div>
							</div>
							<ul class="w-full flex flex-col max-h-[15rem] overflow-y-auto pb-1">
								<li v-for="(countyitem, index) in counties" :key="countyitem.id" @click="selectLocaction(countyitem.id)">
			...
			<div class="submit-btns w-full flex flex-row items-center pt-6 pb-2 justify-between">
				...
				<button
					type="submit"
					class="min-w-[9.1rem] px-8 py-2 text-lg font-bold tracking-wide text-slate-100 bg-orange-600 rounded \ hover:bg-orange-500 transition-colors duration-300 flex items-center justify-center"
				>
					<Transition name="slide" mode="out-in">
