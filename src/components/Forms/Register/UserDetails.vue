...
const submitUserRegistration = async (type: void) => {
	...
	if (response.status === 201) {
		...
		validate.value.validno = true;
		errormessages.value.MobileNumber = fetcherror.value.Error;
		setTimeout(() => {
			submitstatus.value = false;
		}, 500);
	}
		...
		<div class="input-fields w-full sm:w-[80%] flex flex-col gap-y-6 sm:gap-y-6 pt-14 sm:pt-12 transition-all duration-300">
			<div class="input-field w-full flex flex-col gap-y-2">
				...
				<div class="input w-full flex flex-row gap-x-1">
					<div
						class="phone-icon-label px-2 py-2 bg-slate-50 border border-gray-300 w-[20%] rounded-l flex flex-row items-center justify-center gap-x-1 opacity-90"
					>
					...
					<input
						...
						id="phone"
						required
						class="w-[80%] rounded-r placeholder:text-slate-500"
			...
			<div class="input-field w-full flex flex-col gap-y-2">
				...
				<div class="input w-full flex flex-row gap-x-1">
					<input type="text" name="fname" id="fname" required class="w-full rounded" v-model.trim="userdetails.FirstName" />
				</div>
			</div>
			<div class="input-field w-full flex flex-col gap-y-2">
				<label for="lname" class="text-[#346974] text-base font-semibold">Last Name</label>
				<div class="input w-full flex flex-row gap-x-1">
					<input type="text" name="lname" id="lname" required class="w-full rounded" v-model.trim="userdetails.LastName" />
				</div>
			</div>
