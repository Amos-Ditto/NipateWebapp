		...
		<div class="body-section flex flex-col gap-y-2 sm:px-8 text-[#346974]">
			<div class="data w-full" v-if="!emptylist">
				<Transition mode="out-in">
					<div class="adsuspense flex flex-col gap-y-2" v-if="fetchingadssuspense">
						<AdvertAccountSuspense />
						<AdvertAccountSuspense />
						<AdvertAccountSuspense />
					</div>
					<div class="adlist flex flex-col gap-y-2" v-else>
						<AdvertAccount v-for="advert in advertlist" :key="advert.id" :advert="advert" />
					</div>
				</Transition>
			</div>
			<div class="empty w-full text-[#346974]" v-else>
				<div class="empty-container flex flex-col items-center justify-center gap-y-3 pt-6 pb-2 border border-gray-300 rounded">
					<div class="icon w-full flex items-center justify-center py-3">
						<div class="i-mdi-alert-circle-outline text-7xl text-[#98a4a6] scale-125"></div>
					</div>
					<div class="text-desc flex flex-col gap-y-5 items-center justify-center">
						<h3 class="text-xl font-semibold tracking-wide">You have not posted any adverts yet.</h3>
						<button
