...
<template>
	<section class="w-full flex flex-col gap-y-10 sm:gap-y-10">
		...
		<div class="body-section flex flex-col gap-y-12 px-4 sm:px-8 text-[#346974] md:border border-gray-300 py-8 rounded">
			<div class="input-field flex flex-col w-full gap-y-2">
				<label for="title" class="text-sm font-semibold tracking-wide">Ad Title</label>
				<input
					...
					:class="valid_form.title ? 'border-gray-200' : 'border-tomato'"
				/>
			</div>
			<div class="input-field flex flex-col gap-y-2 w-full relative">
				<label for="LocationID" class="text-sm font-semibold tracking-wide">Region/County</label>
				<button
					...
					:class="valid_form.county ? 'border-gray-200' : 'border-tomato'"
				>
					{{ selectedregion.Name }}
					<div class="i-mdi-chevron-down right-4 absolute text-lg"></div>
				</button>
				<Transition name="popup">
					...
					>
						...
						<ul class="flex flex-col gap-y-1 py-2 min-h-[2rem] max-h-[14rem] overflow-y-auto">
							<li
								v-for="county in regions"
								:key="county.id"
								@click="selectRegion(county)"
								:class="region_select_id === county.id && 'bg-gray-100'"
							>
								{{ county.Name }}
							</li>
						</ul>
			...
			<div class="input-field flex flex-col gap-y-2 w-full">
				<label for="desc" class="text-sm font-semibold tracking-wide">Ad Description</label>
				<textarea
					...
					:class="valid_form.description ? 'border-gray-200' : 'border-tomato'"
				></textarea>
			</div>
			<div class="input-fields grid grid-cols-2 gap-x-4 w-full">
				<div class="input-field flex flex-col gap-y-2">
					...
					<input
						...
						:class="valid_form.startdate ? 'border-gray-200' : 'border-tomato'"
					/>
				</div>
				...
				<div class="input-field flex flex-col gap-y-2">
					...
					<input
						...
						id="end"
						class="outline-none bg-white py-2 px-4 w-full md:w-3/4 border rounded tracking-wide"
						:class="valid_form.expirydate ? 'border-gray-200' : 'border-tomato'"
					/>
				</div>
			</div>
