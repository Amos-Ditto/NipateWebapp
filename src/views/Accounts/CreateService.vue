...
import type { ServicesCategoryEntity, ServicesEntity, CreateService, CenterLocation } from "../../Types/GeneralTypes";
				...
				<div class="input-field w-full flex flex-col gap-y-2 relative">
					...
					>
						<ul class="categories py-1 w-full flex flex-col max-h-[10rem] overflow-y-auto" v-if="toggleservices">
							<li v-for="service in servicecategories" @click="chooseServiceCategory(service)">
								<div
									...
									:class="selectedcategory.id === service.id ? 'block' : 'hidden'"
								></div>
								{{ service.Name }}
							</li>
						</ul>
						<SelectService
							...
							@close-service-list="toggleservices = !toggleservices"
							v-else
						/>
					</div>
				</div>
