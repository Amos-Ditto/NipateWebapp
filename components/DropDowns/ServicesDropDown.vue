...
import {Categories} from '~~/types/LocationTypes';
const toggleproductcategories = ref<boolean>(true);

const emit = defineEmits<{
    (event: 'selectCategoryFilterTag', payload:Categories);
}>();
...
const towns: Categories[] = [
    {id: 1, Name: 'Kabarak'}, {id: 2, Name: 'Kiamunyi'}, {id: 3, Name: 'Mercy Njeri'}
]

// Methods
const toggleProductCategory = ():void => {
    toggleproductcategories.value = !toggleproductcategories.value;
}

const selectCategoryFilterTag = (payload: Categories):void => {
    emit("selectCategoryFilterTag", payload);
};
</script>
<template>
    <div class="drop-down">
        <transition name="dropdown">
            <div class="location-group w-full py-2 flex flex-col gap-2" v-if="toggleproductcategories">
                ...
                >
                    <button
                        ...
                        v-for="(category, index) in apicategories" :key="index"
        ...
        <transition name="dropdown">
            <div class="group w-full py-2 flex flex-col gap-2" v-if="!toggleproductcategories">
                <div class="search-group w-full h-[15%] flex items-center px-4 relative">
                ...
                <div class="group-btn flex flex-wrap items-center w-full max-h-[130px] gap-4 px-4 py-4 overflow-y-auto">
                    <button
                        ...
                        @click="selectCategoryFilterTag(town)"
                    >{{town.Name}}</button>
                </div>
            </div>
        </transition>
